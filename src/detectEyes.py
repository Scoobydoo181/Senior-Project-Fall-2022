...
def centerCoordinates(f):
    ...
    def inner(*args, **kwargs):
        eyes = f(*args, **kwargs)
        return [
            map(round, (x + w / 2, y + h / 2))
            for (x, y, w, h) in sorted(eyes, key=lambda eye: eye[1])[:2]
        ]
...
def detectEyes(
    image,
    detectorType=DetectionType.EYE_CASCADE,
    eyeDetector=None,
    blobDetector=None,
    faceDetector=None,
):
...
def eyeCascadeBlobDetector(image, eyeDetector, blobDetector, demo=False):
    ...
    return [
        tupleAdd(pupil[0].pt, x, y)
        if len(pupil) > 0
        else (x + round(w / 2), y + round(h / 2))
        for pupil, (x, y, w, h) in zip(pupils, eyes)
    ]
...
def testMain():
    ...
    while True:
        ...
        eyes = detectEyes(
            image, DetectionType.EYE_CASCADE_BLOB, eyeDetector, blobDetector
        )
        for (x, y) in eyes:
